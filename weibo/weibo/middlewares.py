...
class CookiesMiddleware(object):
    ...
    def _get_random_cookies(self):
        try:
            ...
            import random
            from atlednolispe_weibo import cookies_pool

            return cookies_pool[random.randint(0, len(cookies_pool)-1)]
        ...
        if 299 < response.status < 400:
            try:
                ...
                if 'login.weibo' in redirect_url or 'login.sina' in redirect_url:
                    ...
                    self.logger.warning('Cookies(%s) is locked!' % json.dumps(request.cookies))
